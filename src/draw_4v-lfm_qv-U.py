...
tp  = units.Quantity(lev, "hPa")
qvs = mpcalc.saturation_mixing_ratio(tp, T0) * 1000
qv = qvs * rh0 / 100.0 
...
#print(rh0)
#rh0.plot.contourf(ax=ax,transform=proj)#,levels=clevels,cmap='rainbow',cbar_kwargs={'label':'U-velocity (m s$^{-1}$)'})
...
#---ベクトルを描く-------------------
Usub = u.isel(TIME=it).sel(LEV=lev)#.squeeze()
Vsub = v.isel(TIME=it).sel(LEV=lev)#.squeeze()
...
#!# 以下、Yamashitaさんのページを参考にしたが、県境が描かれない
#!# 10mの解像度のデータ
...
#!shapefile = shapereader.natural_earth(category='cultural',
#!                          resolution='10m',
...
#!    if record.attributes['admin'] == 'Japan']
#!# Define a Cartopy Feature for the province borders, so they can be easily plotted
#!provinces = ShapelyFeature(province_geos, crs=proj,facecolor='none',edgecolor='black',lw=1)
#!#                           crs=ccrs.PlateCarree(),                           
#!                           
#!                           
#!#ax.add_feature(provinces)
#!#ax.add_geometries(provinces, proj)
#!
